...
|------|------|----------------|
| 2026-04-18 | **Audit Sprint 2 (RLS / server trust):** Re-read `supabase-audit-rls-snapshot.json` — syndicate tables already have manager-scoped `INSERT`/`UPDATE`/`DELETE` policies (`is_syndicate_manager`, etc., 2026-04-12 capture). **No new SQL** this session; in-repo record confirms defence-in-depth. **`diary.js`** — block comment + per-mutation `// RLS: <policy>` notes before `syndicate_targets` upsert, `syndicate_member_allocations` upsert, `syndicate_invites` insert/delete, `syndicate_members` role/status updates, `syndicates` delete. Optional manual check: signed in as a **non-manager** member, run a forged `update` on `syndicate_members` in DevTools → expect **RLS error**. | This file, `diary.js`, `PROJECT-LOG.md` |

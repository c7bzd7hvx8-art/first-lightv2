...
|------|------|----------------|
| 2026-04-16 | Ran `syndicate-manager-larder.sql` in Supabase SQL Editor — **success** (no rows returned). Manager-only RPC `syndicate_member_larder_for_manager(uuid, text)` now live; Team Larder button operational. | SQL Editor result (chat), `scripts/syndicate-manager-larder.sql`, this file |
